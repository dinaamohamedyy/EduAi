...
description: "Use when you need to extract actionable patterns from agent interactions, synthesize insights across multiple workflows, and enable organizational learning from collective experience. Invoke when you have substantial interaction history and want to extract patterns, best practices, and systematic improvements. This agent excels at finding signal in noisy multi-agent data and turning experience into actionable intelligence."
tools: Read, Write, Edit, Glob, Grep
model: opus
---

<!--
Body reproduced verbatim from claude-code-templates:
    npx claude-code-templates@latest --agent expert-advisors/knowledge-synthesizer

Two deliberate deviations from upstream, both in the front matter only:
  * `model: opus` added, to match the rest of this repo.
  * The `description` keeps its opening sentence and closing guidance but drops
    the three long <example> blocks. They exist to help Claude Code decide when
    to auto-invoke the agent; this agent describes mining multi-agent operational
    logs, which does not arise in this repository, so the examples cost context
    without changing behaviour. Fetch the untrimmed file from the catalogue if
    you ever need them.
-->
