---
name: test-engineer
description: Test automation and quality assurance specialist — test strategy, coverage analysis, CI/CD quality gates. Invoke by name when a testing pass is wanted; do not auto-select.
tools: Read, Write, Edit, Bash
---

<!--
Upstream: development-tools/test-engineer, from the claude-code-templates
catalogue. Fetched from raw.githubusercontent.com rather than installed with
`npx claude-code-templates@latest --agent development-tools/test-engineer`,
because this machine has no Node (docs/01-setup-guide.md).

Deliberate deviation from the unmodified-upstream convention that
.claude/agents/expert-advisors/ follows: the `description:` line has been
narrowed. Upstream declares "Use PROACTIVELY", and combined with Write and
Bash that makes an auto-selectable agent whose every worked example is Jest,
Playwright, Docker and Postgres — none of which exist in this WordPress repo,
and none of which should be scaffolded into it on the agent's own initiative.
Everything below this comment is byte-for-byte upstream; only the description
changed. Raised by the tech manager, 9 Aug 2026.

What transfers to this project is the method, not the toolchain: the test
pyramid, quality gates, risk assessment and CI enforcement. Applied here that
means scripts/contract-tests.pl, scripts/php-sanity.pl and the lint job in
